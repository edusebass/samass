...
if (isset($_GET['codigo'])) {
    $codigo = "M_" . $_GET['codigo'];
    $query = "SELECT * FROM materiales WHERE codigo = ?";
    $material = ejecutar_query($conn, $query, [$codigo])->fetch(PDO::FETCH_ASSOC);
}
...
if (isset($_POST['submit_item'])) {
    ...
    if (empty($_POST['mat_nombre']) || 
        ...
        try {
            ...
            $foto_path = $material && !empty($material['mat_foto']) ? $material['mat_foto'] : './../../public/ico/material.svg';
            if (isset($_FILES['mat_foto']) && $_FILES['mat_foto']['error'] == UPLOAD_ERR_OK) {
                ...
                move_uploaded_file($_FILES['mat_foto']['tmp_name'], $foto_path);
            }

            // Si es edición (material ya existe), haz UPDATE
            if ($material) {
                ...
                // Mensaje de éxito y redirección en 1 segundo
                echo "<div class='alert alert-success mt-3'>Material actualizado correctamente. Redirigiendo a la ficha...</div>";
                echo "<script>
                    setTimeout(function() {
                        window.location.href = '../pages/ficha-material.php?codigo=" . urlencode(str_replace('M_', '', $material['codigo'])) . "';
                    }, 1000);
                </script>";
                $conn->commit();
                exit;
            ...
            $conn->rollBack();
            $error_message = "Hubo un error al guardar el material: " . htmlspecialchars($e->getMessage());
        }
    }
}
...
function renderInput($name, $details, $material = null) {
    $label = $details['label'];
    $type = $details['type'];
    ...
    if ($type == 'select') {
        ...
        foreach ($options as $option) {
            $opt_value = $option[array_keys($option)[0]];
            $opt_text = $option[array_keys($option)[1]];
            $selected = ($material && $material[$name] == $opt_value) ? 'selected' : '';
            $html .= "<option value='$opt_value' $selected>$opt_text</option>";
        }
        $html .= "</select>";
    } else if ($type == 'textarea') {
        $html .= "<textarea id='$name' class='form-control' name='$name' minlength='5' maxlength='150' $required>$value</textarea>";
    } else {
        $html .= "<input type='$type' class='form-control border-2 rounded-3' id='$name' name='$name' value='" . htmlspecialchars($value) . "' minlength='5' maxlength='200' $required>";
    }
    $html .= "  </div>
            </div>";
    return $html;
...
<title>Editar material | SAM Assistant</title>
</head>
<body>
    <section class="container-fluid mt-3 ">
        <h5 class="w-100 mb-2 bg-plomo p-2"><B>EDITAR MATERIAL</B></h5>
        ...
        <section id="materialesFields" >
                    ...
                    <div class="card rounded-4 px-1 mb-3">
                        <div class="row m-1">
                            ...
                            <div class="col-12 col-sm-6 d-flex flex-column position-relative">
                                <?php
                                echo renderInput('id_materiales', $campos['id_materiales'], $material);
                                echo renderInput('mat_nombre', $campos['mat_nombre'], $material);
                                echo renderInput('mat_descripcion', $campos['mat_descripcion'], $material);
                                ?>
                            </div>
                            <!-- Columna 2 -->
                            <div class="col-12 col-sm-6 d-flex flex-column position-relative">
                                <?php
                                echo renderInput('id_estado', $campos['id_estado'], $material);
                                ?>
                                <div class="row py-2 d-flex">
                                    ...
                                    <div class="col-5  align-items-center">
                                        <?php if ($material && !empty($material['mat_foto'])): ?>
                                            <img id="preview" class="img-thumbnail mt-3 rounded" src="./../../<?= htmlspecialchars($material['mat_foto']) ?>" alt="Vista previa">
                                        <?php else: ?>
                ...
                <div class="col-12">
                    ...
                    <div class="card rounded-4 px-1 mb-3">
                        <div class="row m-1">
                            ...
                            <div class="col-12 col-sm-6 d-flex flex-column position-relative">
                                <?php
                                echo renderInput('id_mat_area', $campos['id_mat_area'], $material);
                                echo renderInput('id_mat_medida', $campos['id_mat_medida'], $material);
                                ?>
                            </div>
                            <!-- Columna 2 -->
                            <div class="col-12 col-sm-6 d-flex flex-column position-relative">
                                <?php
                                echo renderInput('mat_cantidad', $campos['mat_cantidad'], $material);
                                echo renderInput('mat_minimo1', $campos['mat_minimo1'], $material);
                                // echo renderInput('mat_minimo2', $campos['mat_minimo2'], $material);
                                ?>
                            </div>
                        </div>
                    </div>
            ...
            <button class="btn btn-primary my-3" type="submit" name="submit_item">Guardar edición</button>
        </form>
    </section>
